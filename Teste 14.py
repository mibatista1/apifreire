...
class TextAnalyzerApp:
    ...
    def preprocess_text(self, text):
        ...
        # Lematizar os tokens (usando lematização para inglês)
    ...
    def plot_and_show(self, plot_function, title):
        """Gera e exibe um gráfico em uma nova janela Tkinter."""
    ...
    def analyze_similarity(self, text_data):
        def plot_similarity(ax):
            ...
            plt.colorbar(ax.imshow(similarity_matrix, cmap='hot', interpolation='nearest'))
    ...
    def analyze_text_complexity(self, text_data):
        def plot_complexity(ax):
            ...
            ax.hist(complexities, bins=10, color='blue', edgecolor='black')
            ax.set_xlabel("Índice de Facilidade de Leitura")
            ax.set_ylabel("Frequência")
            ax.set_title("Complexidade do Texto")
    ...
    def analyze_word_relationships(self, text_data):
        def plot_relationships(ax):
            ...
            ax.set_title("Relações Entre Palavras")

        self.plot_and_show(plot_relationships, "Relações Entre Palavras")

    def analyze_sentences(self, text_data):
        def plot_sentences(ax):
            sentences = text_data.split('.')
            sentence_lengths = [len(sentence.split()) for sentence in sentences if sentence]
            ax.hist(sentence_lengths, bins=20, color='green', edgecolor='black')
            ax.set_xlabel("Número de Palavras por Frase")
            ax.set_ylabel("Frequência")
            ax.set_title("Comprimento das Frases")
    ...
    def analyze_sentiment(self, text_data):
        def plot_sentiment(ax):
            ...
            ax.bar(['Sentimento'], [sentiment], color='orange')
    ...
    def analyze_word_cooccurrence(self, text_data):
        def plot_cooccurrence(ax):
            ...
            ax.barh(bigrams, counts, color='teal')
            ax.set_xlabel("Frequência")
            ax.set_title("Coocorrência de Palavras")
    ...
    def analyze_ngrams(self, text_data):
        def plot_ngrams(ax):
            ...
            ax.barh(ngrams, counts, color='cyan')
            ax.set_xlabel("Frequência")
            ax.set_title("N-gramas")
    ...
    def analyze_text_classification(self, text_data):
        def plot_classification(ax):
            ...
            ax.bar(['Classificação'], [prediction], color='red')
            ax.set_ylabel("Classe")
            ax.set_title("Classificação do Texto")
